...
// ///////////////////////////////////////////////////////// Format a time range
// Takes a start date and an end date and outputs a time range without identical
// values in each respective time order. ------------------------------------ //
// i.e. January 1-3 2022 or January 1 2022 - January 3 2023 ----------------- //
const FormatTimeRange = (start, end) => {
  const startDate = moment(start).format('MMM D YYYY').split(' ')
  const endDate = moment(end).format('MMM D YYYY').split(' ')
  if (startDate[2] !== endDate[2]) { return `${startDate.join(' ')} - ${endDate.join(' ')}` }
  if (startDate[0] !== endDate[0]) { return `${startDate[0]} ${startDate[1]} - ${endDate[0]} ${endDate[1]} ${startDate[2]}` }
  if (startDate[1] !== endDate[1]) { return `${startDate[0]} ${startDate[1]}-${endDate[1]} ${startDate[2]}` }
...
export default ({ store }, inject) => {
  ...
  inject('FormatTimeRange', FormatTimeRange)
}
